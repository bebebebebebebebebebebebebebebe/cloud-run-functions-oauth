...
from app.auth.token import create_access_token, create_refresh_token, decode_token, verify_token
...
def test_create_access_token_payload():
    ...
    token = create_access_token(data)
    payload = jwt.decode(
        token, settings.JWT_SECRET_KEY, algorithms=[settings.SIGNATURE_ALGORITHM]
    )
    ...
    expected_expiry = datetime.now(tz=ZoneInfo("Asia/Tokyo")) + timedelta(
...
def test_create_and_decode_with_custom_key():
    ...
    custom_settings.JWT_SECRET_KEY = "custom_test_secret_key"
...
def test_create_with_custom_expiry():
    ...
    payload = jwt.decode(
        token, custom_settings.JWT_SECRET_KEY, algorithms=[custom_settings.SIGNATURE_ALGORITHM]
    )
    expected_expiry_timestamp = (
        datetime.now(tz=ZoneInfo("Asia/Tokyo"))
        + timedelta(minutes=custom_settings.ACCESS_TOKEN_EXPIRE_MINUTES)
    ).timestamp()
...
def test_verify_with_custom_key_failure():
    ...
    custom_settings = settings.model_copy(deep=True)
    custom_settings.JWT_SECRET_KEY = "a_very_different_secret_key_for_failure_test"
...
def test_refresh_token_with_custom_settings():
    ...
    custom_settings.JWT_SECRET_KEY = "custom_refresh_secret_key"
    ...
    expected_expiry_timestamp = (
        datetime.now(tz=ZoneInfo("Asia/Tokyo"))
        + timedelta(days=custom_settings.REFRESH_TOKEN_EXPIRE_DAYS)
    ).timestamp()
...
def test_verify_token_invalid_signature():
    ...
    valid_payload = {"sub": "testuser@example.com", "exp": datetime.now(tz=ZoneInfo("Asia/Tokyo")) + timedelta(minutes=15)}
    token_signed_correctly = jwt.encode(valid_payload, settings.JWT_SECRET_KEY, algorithm=settings.SIGNATURE_ALGORITHM)
...
def test_decode_token_invalid_signature():
    ...
    settings.JWT_SECRET_KEY = "another_secret_key"
    # Re-encode with a different key to make it invalid for the original key
    invalid_token_payload = {"sub": "testuser@example.com", "email": "testuser@example.com", "exp": datetime.now(tz=ZoneInfo("Asia/Tokyo")) + timedelta(minutes=15)}
    invalid_token = jwt.encode(invalid_token_payload, "another_secret_key_to_make_it_fail", algorithm=settings.SIGNATURE_ALGORITHM)
...
def test_create_access_token_custom_expiry():
    ...
    expected_expiry_timestamp = (
        datetime.now(tz=ZoneInfo("Asia/Tokyo")) + custom_delta
    ).timestamp()
    assert abs(payload["exp"] - expected_expiry_timestamp) < 5  # Allow 5s difference
